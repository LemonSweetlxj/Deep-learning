...
The task is using DQN to realise the cartpole
According to the algorithm, deep Q Network conbine the strength of Q learning and
    Neural Network. The core function of this algorithm is y[i] = reward[i] + GAMMA * maxQ( fai[j+1],a ; theta-)
    This function can be seen in the code.
We use 3 layer network (2 convolution layers and 1 output layer) and the activate function is relu.
The most difficult thing is  parameter setting, and this cost our most of time.
    ...
    and finally we choose the best one with batch_size = 128 ,hidden_nodes = 20 ,learning_rate = 0.001.
The final result:
    1000 times epsilon, the ave reward can achieve around 150.
Another problem that we have not solved is the stablility of the model.
    Most of time we can achieve this goal but a few times the ave reward is low
    after 1000 times epsilon.
"""
"""
Hyper Parameters
"""
...
def init_session():
    ...
    logdir = "tensorboard/" + datetime.datetime.now().strftime(
        "%Y%m%d-%H%M%S_h=20_car_100_layer=2") + "/"
