...
"""
This task was finished by Liyuan Song and Xinjie LU on 28/10.

The task is using Double DQN to realize two environment: mountain car and Pendulum
According to the algorithm, the difference between double DQN and DQN is the function to calculate the target value.
    The target function of DQN is y[i] = reward[i] + GAMMA * maxQ( fai[j+1],a ; theta-)
    in this algorithm, it regard y[i] as index, and put it in current network
    This function can be seen in the code.
We use 4 layer network
    2 convolution layers and 1 output layer, this output layer used to calculate loss and another
    output layer is to store the state. The activate function is relu.
The most difficult thing is  parameter setting, and this cost our most of time.
    The batch_size, hidden_nodes and learning_rate have big influences on the model,
    and finally we choose the best one with batch_size =256 ,hidden_nodes = 10,learning_rate = 0.001.
The final result:
    800 times epsilon, the ave reward of Pendulum can achieve around -330.
    and the ave reward of moutain car can achieve around -190.
    which seems that the double DQN has no big improvement on mountian car.
"""
"""
Hyper Parameters
"""
...
EPSILON_DECAY_STEPS = 100  #STEP = 200
REPLAY_SIZE = 10000  # experience replay buffer size
BATCH_SIZE = 256  # size of minibatch

TEST_FREQUENCY = 10  # How many episodes to run before visualizing test accuracy  #TEST = 10
...
def init(env, env_name):
    ...
    action spaces
    """
    global replay_buffer, epsilon,is_continuous,value_min,value_max
    
    replay_buffer = [] #reply_memory
    epsilon = INITIAL_EPSILON
    is_continuous = not hasattr(env.action_space, 'n')
    if is_continuous:
        value_min = env.action_space.high[0]
        value_max = env.action_space.low[0]

    state_dim = env.observation_space.shape[0] ##input_size
    
    if is_continuous:
        action_dim = ACTION_SPACE
    else:
        action_dim = env.action_space.n ##output_size
...
def get_network(state_dim, action_dim, hidden_nodes=HIDDEN_NODES):
    ...
    action_in = tf.placeholder("float", [None, action_dim])  # one hot targetQ
    target_in = tf.placeholder("float",      # y_input
    ...
    h1_layer = tf.nn.relu(tf.matmul(state_in, W1) + b1)
    
    W2 = weight_variable([HIDDEN_NODES,HIDDEN_NODES])
    b2 = bias_variable([HIDDEN_NODES])

    h2_layer = tf.nn.relu(tf.matmul(h1_layer, W2) + b2)

    W3 = weight_variable([HIDDEN_NODES,action_dim])
    b3 = bias_variable([action_dim])

    global q_values,q_next
    q_values = tf.matmul(h2_layer, W3) + b3
    q_next = tf.matmul(h2_layer, W3) + b3
...
def init_session(): ###init session
    global session, writer
    session = tf.InteractiveSession()
    session.run(tf.global_variables_initializer())

    # Setup Logging  ####loading networks
...
def deal_with_action(discrete_values, min, max):
    range = max - min
    parts = ACTION_SPACE - 1
    value = min + (discrete_values / parts * range)
    return value

def get_env_action(action, q_values,state,state_in):
 #   action = np.argmax(q_values.eval(feed_dict = {state_in:[state]})[0])
    """
    Modify for continous action spaces that you have discretised, see hints in
    `init()`
    """
    if is_continuous:
        return [deal_with_action(action, value_min, value_max)]
    else:
...
def get_train_batch(q_values, state_in, replay_buffer):
    ...
    Q_value_batch = q_next.eval(feed_dict={
        state_in: next_state_batch
    })
    

    Q_value_batch3 = q_values.eval(feed_dict={
        state_in: state_batch
    })
    ...
    for i in range(0, BATCH_SIZE):
        ...
        if sample_is_done:
            ...
            target_val = reward_batch[i] + GAMMA * Q_value_batch[i][np.int32(np.argmax(Q_value_batch3[i]))]
    ...
    for episode in range(num_episodes):
        ...
        for step in range(ep_max_steps):
            ...
            action = get_action(state, state_in, q_values, epsilon, test_mode,
                                action_dim)
            env_action = get_env_action(action, q_values,state,state_in)
