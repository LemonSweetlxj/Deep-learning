...
##after 5 epoch, the test accuracy up to 92%--93%
def onelayer(X, Y, layersize=10):
    ...
    #lose function
    batch_xentropy = -1 * Y * tf.log(preds)
    batch_loss = tf.reduce_mean(tf.reduce_sum(batch_xentropy,reduction_indices = [1]))
...
#after 5 enpoch, the test accuracy up to 96% -- 97%
def twolayer(X, Y, hiddensize=30, outputsize=10):
    #model: y = w * first-layer-output+ b
    #the first layer weight
    w1 = tf.Variable(tf.truncated_normal([784,hiddensize],stddev = 0.1))
    #the first lalyer bias
    b1 = tf.Variable(tf.zeros([hiddensize]))
    #the first layer output
    h1 = tf.nn.relu(tf.matmul(X,w1) + b1)
    #the second layer weight
    w2 = tf.Variable(tf.truncated_normal([hiddensize,10],stddev = 0.1))
    #the second layer bias
    b2 = tf.Variable(tf.zeros([10]))
    #the second layer output == the input of activation function
    logits = tf.matmul(h1,w2) + b2
    #the output of activation function
    preds = tf.nn.softmax(logits)
    #to avoid gradient explosion
    preds_ = tf.clip_by_value(preds,1e-8,1.0)
    #loss function
    batch_xentropy = -1 * Y * tf.log(preds_)
    batch_loss = tf.reduce_mean(tf.reduce_sum(batch_xentropy,reduction_indices = [1]))
...
##after 5 epoch, the test accuracy up to 98%
def convnet(X, Y, convlayer_sizes=[10, 10], \
        filter_shape=[3, 3], outputsize=10, padding="same"):
    #model:conv_layer1 → conv_layer2 → fully-connected → output
    ...
    h_pool2_flat = tf.reshape(conv2,[-1, 28*28*outputsize])
    
    #output layer
    w = weight_variable([28*28*outputsize,outputsize])
    b = bias_variable([outputsize])

    #the fully connected layer output = the input of activation function
    logits = tf.matmul(h_pool2_flat,w) + b
    #the output of activation function
    preds = tf.nn.softmax(logits)
    #to avoid gradient explosion
    preds_ = tf.clip_by_value(preds,1e-8,1.0)
